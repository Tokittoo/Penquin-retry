  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {extensions.map((extension, index) => (
          <motion.div
            ...
            className="group relative overflow-hidden rounded-lg border border-border bg-background/50 backdrop-blur-sm p-6 hover:bg-background/80 transition-all duration-300 hover:shadow-lg hover:shadow-primary/10"
          >
      ...
      <motion.div
        ...
        className="mt-12 grid grid-cols-1 md:grid-cols-3 gap-6 p-6 rounded-lg border border-border bg-muted/50"
      >
        <div className="text-center">
          <div className="text-2xl font-bold text-primary">{extensions.length}</div>
          <div className="text-sm text-muted-foreground">Total Extensions</div>
        </div>
        <div className="text-center">
          <div className="text-2xl font-bold text-primary">
            {extensions.filter(ext => ext.store.includes('Firefox')).length}
          </div>
          <div className="text-sm text-muted-foreground">Firefox Extensions</div>
        </div>
        <div className="text-center">
          <div className="text-2xl font-bold text-primary">
            {extensions.filter(ext => ext.store.includes('Github')).length}
          </div>
